...
export default function SelectRepo() {
    ...
    async function querySetWorkingDir() {
        ...
        const url = '/system-environment/setWorkingDir' + makeParams({WorkingDir: newWorkingDir});
        try {
    ...
    return (
        <React.Fragment>
            ...
            <table id="repoTable">
                ...
                <tr>
                    <td>server</td>
                    <td id="serverTable" className="left"> {rangData.server}</td>
                </tr>
                <tr>
                    <td>directory</td>
                ...
                <tr>
                    <td>WorkingDir</td>
                    <td id="workingDir-row" className="left"> {rangData.workingDir}</td>
                </tr>
                {rangData2.repoName.map((repo, index) => {
                    return (
                        <tr  key={index}>
                            <td>Repo Name</td>
                            <td id={"repoName-row" + index }>{repo}</td>
                        </tr>
                    );
                })}
                </tbody>
            ...
            <div>
                ...
                <button id="setDirButton" onClick={querySetWorkingDir}>Set Working Dir</button>
            </div>
