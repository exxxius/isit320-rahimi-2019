...
//let workingDir = getWorkingDir();//'isit320-rahimi-2019';
let workingDir = 'git-ignore-tests';
const badFiles = [
    'bower_components',
    'npm-debug.log',
    'bundle.js',
...
async function getRepoNames(response) {
    //return {"result": "success"};
    const plan = 'find . -maxdepth 2 -iname ".git" -type d -prune';
    const { stdout, stderr } = await exec(plan);
    const output = [];
    stdout.split('\n').forEach(element => output.push(element.substr(2, element.length - 7)));
    console.log('stdout:', output);
    console.error('stderr:', stderr);
    if (response) {
        response.send({ repoName: output.filter(name => name != '') });
    } else {
        return output;
    }
}
...
function setWorkingDir(response) {
    //workingDir = getRepoNames();
    //response.send({workingDir: 'isit320-rahimi-2019'});
    workingDir = response;
    console.log('EXECGIT SETWORKINGDIR CALLED: ', workingDir);
}
...
module.exports.getRepoNames = getRepoNames;
module.exports.setWorkingDir = setWorkingDir;
